...
@HiltViewModel
class MainDoctorViewModel @Inject constructor(
    ...
    fun acceptAndSendTrainees(traineeUserId: String, trainerUserId: String) {
        ...
        Log.d("DoctorsViewModel", "Checking if trainee request document exists...")
        traineeRequestDocRef.get().addOnSuccessListener { traineeRequestSnapshot ->
            if (traineeRequestSnapshot.exists()) {
                Log.d("DoctorsViewModel", "Trainee request document exists. Proceeding with batch operation...")
